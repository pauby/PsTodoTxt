...
task InstallRequiredModules {
    ...
    $required = 'PSScriptAnalyzer', 'Pester', 'PSCodeHealth'
    try {
        ...
        $required | Where-Object { -not (Get-Module $_ -ListAvailable) } | ForEach-Object {
...
task UpdateAppVeyorVersion -If (Test-Path (Join-Path $psscriptroot 'appveyor.yml')) {
...
task PSScriptAnalyzer -If (Get-Module PSScriptAnalyzer -ListAvailable) {
    try {
        Push-Location $buildInfo.Path.Source
        $customRules = if (Test-Path 'CustomAnalyzerRules' -PathType Container) { 'CustomAnalyzerRules\*.psd1' } else { '.' }
         write-host "custom rules: $customRules"
        'priv*', 'pub*', 'InitializeModule.ps1' | Where-Object { Test-Path $_ } | ForEach-Object {
            $path = Resolve-Path (Join-Path $buildInfo.Path.Source $_)
            if (Test-Path $path) {
                Invoke-ScriptAnalyzer -Path $path -Recurse -CustomRulePath $customRules -IncludeDefaultRules -v | ForEach-Object {
                    $_
...
task TestModule {
    ...
    $pester = Start-Job -ArgumentList $buildInfo -ScriptBlock {
        ...
        Import-Module $buildInfo.Path.Manifest -Global -ErrorAction Stop -Force
        $params = @{
            Script       = $path
